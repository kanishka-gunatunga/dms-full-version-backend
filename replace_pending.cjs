...
const replacement = `            // NEW LOGIC: Check levels
            ...
            $documentsQuery = Documents::whereIn('category', $assignedCategoryIds)
                ->where(function ($query) {
                    $query->where('is_archived', 0)
                          ->orWhereNull('is_archived');
                })
...
if (content.includes(target)) {
    ...
    console.log('Target not found! Dumping similar area...');
    const match = content.match(/public function pending_signatures[\s\S]*?->get\(\);/);
