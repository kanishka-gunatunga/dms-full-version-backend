    ...
    public function initialize_chat($id, Request $request)
{
    ...
    basename($file_path_full) // <-- FIX
        ...
        // $documentText =$response->json()['pages'] ?? [];
      $data = $response->json();
$pages = $data['pages'] ?? [];
...
// Extract text only
$textArray = array_map(function ($page) {
    return $page['text'] ?? '';
}, $pages);
...
$documentText = implode("\n\n", $textArray);
        ...
        if ($send_all_to_pinecone == 1) {
            $usePinecone = true;
        } elseif ($set_page_limit == 1 && $documentContext['pageCount'] > $pages_count) {
            $usePinecone = true;
        }
