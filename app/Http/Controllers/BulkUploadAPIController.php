            ...
            if ($request->copy_files_from_computer == 1) {
                ...
                foreach ($rows as $row) {
                    ...
                    if ($fileName === $expectedFileName) {
                        ...
                        if (strtolower(trim($ftp_host)) === 'local') {
                            if (file_put_contents($ftp_file_path, $stream)) {
                                $upload_success = true;
                            }
                        } else {
                        ...
                        if (!$upload_success) {
                            ...
                            return response()->json([
                                'status' => "fail",
                                'message' => "Failed to upload file: $expectedFileName to the " . ($ftp_host === 'local' ? 'local folder' : 'FTP server') . "."
                            ], 500);
                        }
                        fclose($stream);
                ...
                foreach ($rows as $row) {
                    ...
                    $version->file_path = $file_path . $record['name'];
                    $version->date_time = $date_time;
                    $version->user = $request->user;
                    $version->save();
                }
                ...
                if ($document->uploaded_method == 'ftp') {
                    ...
                    if (strtolower(trim($ftp_host)) === 'local') {
                        ...
                        if (file_exists($filePath)) {
                            ...
                            // Fallback: search in root_path
                            $fullLocalPath = rtrim($ftp_root, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . ltrim($filePath, '/\\');
                    ...
                    if (!$retrieved) {
                        throw new \Exception("Failed to retrieve file from " . (strtolower(trim($ftp_host)) === 'local' ? 'local folder' : 'FTP server') . ".");
                    }
