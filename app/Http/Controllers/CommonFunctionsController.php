...
function getAccessToken()
{
    ...
    $response = Http::withoutVerifying()->asForm()->post(
        "https://login.microsoftonline.com/{$credential_details->tenant_id}/oauth2/v2.0/token",
        [
...
function getUsers()
{
    ...
    $allUsers = [];
    $url = 'https://graph.microsoft.com/v1.0/users?$top=999&$select=id,displayName,mail,userPrincipalName,mobilePhone';
    ...
    if ($document->uploaded_method == 'ftp') {
        ...
        if (strtolower(trim($ftp_account_details->host)) === 'local') {
            ...
            if (file_exists($filePath)) {
                ...
                // Fallback: check if it's relative to root_path
                $fullLocalPath = rtrim($ftp_account_details->root_path, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . ltrim($filePath, '/\\');
                if (file_exists($fullLocalPath)) {
                    $localDisk->put(basename($filePath), file_get_contents($fullLocalPath));
                    $filePath = basename($filePath);
                }
            }
        } else {
            ...
            $localDisk = Storage::disk('local');
            $localDisk->put(basename($filePath), $disk->get($filePath));
            $filePath = basename($filePath);
        }
...
public function retreive_old_document($documentVersion)
{
    $date_time = Carbon::now()->format('Y-m-d H:i:s');

    // Always get main document for FTP + category info
    $mainDocument = $documentVersion->document;
    ...
    if ($mainDocument->uploaded_method == 'ftp') {

        $ftp_account = Categories::where('id', $mainDocument->category)->value('ftp_account');

        if (!$ftp_account) {
            $ftp_account_details = FTPAccounts::where("is_default", 1)->first() ?? FTPAccounts::first();
        } else {
            $ftp_account_details = FTPAccounts::where("id", $ftp_account)->first();
        }
        ...
        $file_type = $documentVersion->type;
        $isImage = in_array($file_type, ['jpeg', 'png', 'gif', 'webp', 'jpg']);
        ...
        return $localDisk->temporaryUrl($filePath, now()->addHour());
    }
    else {
        // Local storage
        $filePath = $documentVersion->file_path;
        ...
        if ($isImage) {
            ...
            return Storage::disk('local')->temporaryUrl($watermarkFilePath, now()->addHour());
        }

        return Storage::disk('local')->temporaryUrl($filePath, now()->addHour());
    }
}
...
public function sendToNode($filePath)
    ...
    public function getAzureADUserInfo($accessToken)
{
    try {
        $response = Http::withoutVerifying()
            ->withToken($accessToken)
        ...
        // Get full user details (important!)
        $userResponse = Http::withoutVerifying()
            ->withToken($accessToken)
            ->get("https://graph.microsoft.com/v1.0/users/{$me['id']}?\$select=id,displayName,userPrincipalName,accountEnabled,givenName,surname,mail,mobilePhone");
